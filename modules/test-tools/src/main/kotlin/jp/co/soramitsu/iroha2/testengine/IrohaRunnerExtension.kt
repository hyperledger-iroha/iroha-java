...
class IrohaRunnerExtension :
    ...
    private suspend fun initIfRequested(extensionContext: ExtensionContext): List<AutoCloseable> =
        coroutineScope {
            val withIroha =
            ...
            val withIrohaManual =
                extensionContext.element
                    .get()
                    .annotations
                    .filterIsInstance<WithIrohaManual>()
                    .firstOrNull()
    ...
    private fun WithIrohaManual.init(extensionContext: ExtensionContext) {
        ...
        val urls =
            when (this.dockerComposeFile.isEmpty()) {
    ...
    private fun File.readDockerComposeData(): List<String>? {
        ...
        val all =
            runCatching {
                yaml
                    .load<Map<String, *>>(this.inputStream())["services"]
                    ?.cast<Map<String, *>>()
                    ?.values
                    ?.toList()
    ...
    private suspend fun createContainers(
        ...
        coroutineScope {
            ...
            val peerIds =
                keyPairs.mapIndexed { i: Int, kp: KeyPair ->
                ...
                async {
                    ...
                    val container: IrohaContainer =
                        IrohaContainer {
                            ...
                            this.envs =
                                withIroha.configs.associate { config ->
    ...
    private fun genesisInstance(clazz: KClass<out Genesis>): Genesis =
        clazz.createInstance().let { genesis ->
            val tx =
                genesis.transaction.copy(
