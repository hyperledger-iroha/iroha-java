 ...
 */
abstract class WrapperResolver<T : Type>(
    open val wrapperName: String,
) : Resolver<T> {
    override fun resolve(
        name: String,
        typeValue: Any?,
...
abstract class SortedWrapperResolver<T : IterableType>(
    ...
    override fun resolve(
        ...
        super.resolve(name, typeValue, schemaParser)?.also {
...
object EnumResolver : Resolver<EnumType> {
    override fun resolve(
        ...
        if (typeValue is Map<*, *> && typeValue["Enum"] != null) {
...
object TupleStructResolver : Resolver<TupleStructType> {
    override fun resolve(
        ...
        if (typeValue is Map<*, *> && typeValue["Tuple"] != null) {
...
object StructResolver : Resolver<StructType> {
    override fun resolve(
        ...
        if ((typeValue is Map<*, *> && typeValue["Struct"] != null)) {
...
object OneStringStructResolver : Resolver<StructType> {
    override fun resolve(
        ...
        if (typeValue is String) {
            ...
            val properties =
                LinkedHashMap<String, TypeNest>().also { map ->
    ...
    private fun String.toFieldName() =
        this.replaceFirstChar { it.lowercase() }.let { name ->
