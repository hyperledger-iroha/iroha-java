...
class PeerTest : IrohaTest<AdminIroha2Client>() {
    ...
    @Test
    @WithIroha([AliceCanManagePeers::class], amount = PEER_AMOUNT)
    fun `registered peer should return consistent data`(): Unit =
        runBlocking {
            val p2pPort = DEFAULT_P2P_PORT
            val alias = "iroha$p2pPort"
            val keyPair = generateKeyPair()
    ...
    fun `round-robin load balancing test`(): Unit =
        runBlocking {
    ...
    private suspend fun isPeerRegistered(
        payload: ByteArray,
    ...
    private fun IrohaContainer.extractPeer() =
        Peer(
            SocketAddr.Host(
                SocketAddrHost(
                    this.config.alias,
