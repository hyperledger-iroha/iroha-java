...
class QueriesTest : IrohaTest<Iroha2Client>() {
    ...
    fun `find all accounts with filter`(): Unit = runBlocking {
        val filter = CompoundPredicateOfAccount.Or(
            listOf(
    ...
    fun `find accounts by domain ID`(): Unit = runBlocking {
        val byDomainIdFilter = CompoundPredicateOfAccount.Atom(
            AccountProjectionOfPredicateMarker.Id(
...
//   @Test
    ...
    fun `find assets by name`(): Unit = runBlocking {
        val byNameFilter = CompoundPredicateOfAsset.Atom(
    ...
    fun `find assets by domain name and asset definition id`(): Unit = runBlocking {
        val byAccountDomainIdAndAssetDefinitionIdFilter = CompoundPredicateOfAsset.And(
            listOf(
    ...
    fun `find asset quantity by id`(): Unit = runBlocking {
        ...
        assertEquals(XorAndValAssets.XOR_QUANTITY, asset.value.cast<AssetValue.Numeric>().numeric.asBigDecimal())
    }
    ...
    fun `find all domains`(): Unit = runBlocking {
        client.submit(QueryBuilder.findDomains())
            .also { domains ->
    ...
    fun `find all domains with filter`(): Unit = runBlocking {
        val filter = CompoundPredicateOfDomain.Atom(
    ...
    fun `pagination works correct after inserting some new accounts`(): Unit = runBlocking {
        ...
        client.submit(
            QueryBuilder.findAccounts(byDomainIdFilter)
...
//    }
    ...
    private suspend fun createAccount(domainId: DomainId = DEFAULT_DOMAIN_ID, metadata: Map<Name, Json> = mapOf()) {
