...
class TriggersTest : IrohaTest<Iroha2Client>() {
    ...
    fun `pre commit trigger should mint asset to account for every transaction`(): Unit =
        runBlocking {
            ...
            client
                .submit(Transfer.asset(DEFAULT_ASSET_ID, BigDecimal(100), BOB_ACCOUNT_ID))
                .also { d ->
                    withTimeout(txTimeout) { d.await() }
                }
    ...
    fun `time trigger execution repeats few times`(): Unit =
        runBlocking {
    ...
    fun `wasm trigger to mint nft for every user and update trigger metadata`(): Unit =
        runBlocking {
            ...
            client
                .submit(SetKeyValue.trigger(triggerId, testKey, testValue))
                .also { d ->
                    withTimeout(txTimeout) { d.await() }
                }
            ...
            client
                .submit(QueryBuilder.findTriggerById(triggerId))!!
                .also {
                    assertEquals(
                        testValue,
                        it.action.metadata.sortedMapOfName[testKey]!!
                            .readValue(),
                    )
                }
        }
    ...
    fun `wasm trigger to mint nft for every user on update trigger metadata event`(): Unit =
        runBlocking {
            ...
            client
                .submit(QueryBuilder.findTriggerById(wasmTriggerId))!!
                .also {
                    assertEquals(
                        testValue,
                        it.action.metadata.sortedMapOfName[testKey]!!
    ...
    fun `unregister executable trigger`(): Unit =
        runBlocking {
            ...
            client
                .submit(Unregister.trigger(triggerId))
                .also { d ->
                    withTimeout(txTimeout) { d.await() }
                }
    ...
    private suspend fun sendAndWait10Txs() {
        repeat(10) { i ->
            client
                .submit(SetKeyValue.account(ALICE_ACCOUNT_ID, "key$i".asName(), "value$i"))
                .also { d ->
                    delay(1000)
                    withTimeout(txTimeout) { d.await() }
                }
    ...
    private suspend fun createNewAsset(
        assetName: String,
        prevSize: Int,
    ) {
        val newAsset = AssetDefinitionId(DEFAULT_DOMAIN_ID, assetName.asName())
        client
            .submit(Register.assetDefinition(newAsset, AssetType.numeric()))
            .also { d ->
                withTimeout(txTimeout) { d.await() }
            }
    ...
    private suspend fun checkDefaultAssetQuantity(): Long =
        client
            .submit(QueryBuilder.findAssetById(DEFAULT_ASSET_ID))!!
            .value
            .cast<AssetValue.Numeric>()
            .numeric
            .asLong()
    ...
    private suspend fun keepNetworkBusyAndCheckAssetDefinitionIds() {
        ...
        repeat(5) { i ->
            client
                .submit(SetKeyValue.account(ALICE_ACCOUNT_ID, "test$i".asName(), "test$i"))
                .also { d ->
        ...
        client
            ...
            .also { assets ->
                ...
                assert(
                    assets.any {
                        expectedDefinition.asString().lowercase() ==
                            it.id.definition
                                .asString()
                                .lowercase()
                    },
