...
public class CanRegisterDomain : ModelPermission {
    public companion object : ScaleReader<CanRegisterDomain>, ScaleWriter<CanRegisterDomain> {
        override fun read(reader: ScaleCodecReader): CanRegisterDomain = try {
            CanRegisterDomain()
        } catch (ex: Exception) {
            throw wrapException(ex)
        }

        override fun write(writer: ScaleCodecWriter, instance: CanRegisterDomain): Unit = try {
        } catch (ex: Exception) {
            throw wrapException(ex)
        }

        public fun equals(o1: CanRegisterDomain, o2: Any?): Boolean = when (o2) {
            null -> false
