...
fun String.asAccountId() =
    this
        .split(ACCOUNT_ID_DELIMITER)
        .takeIf {
            it.size == 2
        }?.let { parts ->
...
fun String.asAssetDefinitionId() =
    this
        .split(ASSET_ID_DELIMITER)
        .takeIf {
            it.size == 2
        }?.let { parts ->
 ...
 */
fun PrivateKey.signAs(message: ByteArray): ByteArray =
    try {
...
fun AccountId.asString(withPrefix: Boolean = true) =
    this.signatory.payload.toHex(withPrefix) +
...
fun InstructionBox.extractAccount() =
    this
...
fun InstructionBox.Register.extractAccount() =
    this
        .registerBox
        .cast<RegisterBox.Account>()
        .registerOfAccount.`object`

fun InstructionBox.Register.extractDomain() =
    this
        .cast<InstructionBox.Register>()
        .registerBox
...
fun InstructionBox.Register.extractAssetDefinition() =
    this
        .cast<InstructionBox.Register>()
        .registerBox
...
inline fun <reified I : InstructionBox> SignedTransaction.extractInstruction(): I =
    this
        .cast<SignedTransaction.V1>()
        .extractInstruction<I>()

inline fun <reified I : InstructionBox> SignedTransaction.V1.extractInstruction() =
    this
        .extractInstructionVec<I>()
        .first()
        .cast<I>()

inline fun <reified I : InstructionBox> SignedTransaction.V1.extractInstructions() =
    this
        .extractInstructionVec<I>()
        .cast<List<I>>()

inline fun <reified I : InstructionBox> SignedTransaction.V1.extractInstructionVec() =
    this
...
fun InstructionBox.SetKeyValue.extractDomainId() =
    this
...
fun InstructionBox.Grant.extractValuePermissionToken() =
    this
...
fun EventFilterBox.extractSchedule() =
    this
...
fun BlockMessage.extractBlock() =
    this.signedBlock
        .cast<SignedBlock.V1>()
        .signedBlockV1.payload
...
fun FindError.extract() =
    when (this) {
        ...
        is FindError.Transaction ->
            this.hashOf.hash.arrayOfU8
                .toHex()
    }

fun Number.asNumeric() =
    when (this) {
