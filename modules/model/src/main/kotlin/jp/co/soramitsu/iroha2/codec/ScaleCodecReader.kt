...
class ScaleCodecReader(private val source: ByteArray) {
    ...
    inline fun <reified T : Any> readNullable(): T? = when (T::class) {
     ...
     */
    inline fun <reified T> readArray(size: Int, supplier: () -> T): Array<T> = Array(size) { supplier() }

    /**
     * Read a vector of a specified [size]
     */
    fun <T> readVec(size: Int, supplier: () -> T): List<T> = List(size) { supplier() }
    ...
    private fun capacity(expectedSize: Int): Int = when {
